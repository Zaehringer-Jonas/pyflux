...
#        print("here", str(val))
#        self.intensityEdit.QLineEdit = str(self.intensitySlider.value())
...
#            print("pl lifetime RGBA ",image.shape)1
            if image.shape[-1] == 4:
    ...
    def select_ROI(self):
        ...
        if self.lastScanMode == currentScanMode or self.lastScanMode  == '':
            ...
            if currentScanMode == 'xy':
                ...
                #HACK:
                #newPos_µm[1] = newPos_µm[1] + 12*pxSize
            ...
            if currentScanMode == 'xz':
                ...
                #HACK:
                #newPos_µm[1] = newPos_µm[1] + 15*pxSize
                newPos_µm = np.around(newPos_µm, 3)
                newRange_px = ROIsize[1]
                newRange_µm = pxSize * newRange_px
...
#        self.moveToROIcenterButton.clicked.connect(self.select_ROI)
        ...
        self.initialPosLabel = QtGui.QLabel('Initial Pos'
                                            ' [x0, y0, z0] (µm)')
        self.initialPosEdit = QtGui.QLineEdit('20 20 5')
        ...
        self.NofPixelsEdit = QtGui.QLineEdit('120')
        ...
        self.scanODEdit = QtGui.QLineEdit('2')
...
#            
        ...
        #Shutter Bits
        self.List_Bit   = [self.BitTask1,self.BitTask2, self.BitTask3, self.BitTask4]
...
#        print("lfflag ", self.lifetimeflag)
    ...
    def calculate_derived_param(self):
        ...
        if self.scantype == 'xy':
            ...
            """
            #from middle of image
            self.data_x = np.arange(self.initialPos[0] - self.scanRange/2, self.initialPos[0] + self.scanRange/2,\
                                self.scanRange / self.NofPixels)
            self.data_y = np.arange(self.initialPos[1] - self.scanRange/2, self.initialPos[1] + self.scanRange/2,\
                                self.scanRange / self.NofPixels) 
            """
            self.data_x = np.arange(self.initialPos[0], self.initialPos[0]+ self.scanRange,\
                                self.scanRange / self.NofPixels)
...
#        self.viewtimer_time = (1/1000) * self.data_t[-1]    # in ms
        ...
        self.lifetimeimage = np.zeros(sizelf)
        self.i = 0
...
#        self.gui.currentFrameButton.setChecked(False)
    ...
    def fastline_scan(self, timearr, xpos, ypos ): #xy -> -yx
        ...
        self.set_moveToXY(xpos , ypos[0], t = 0.05)
        #print(ypos[0],ypos[-1])
        ...
        self.set_moveToXY(xpos , ypos[-1], t = 0) 
        
        time.sleep(0.008) #Offsets Time Delay between Movement beginning and TCSPC Measurement(measured by ROI differences)#
#        lgwak  = 0
        ...
        while TCSPCdone == False:
            try:
                TCSPCdone, count, reltimes = self.scanTCSPCdata.get(timeout=0)
        ...
        return line_data[0], colorcode, count, reltimes
    ...
    def fastline_scan_xz(self, timearr, xpos, ypos ): #xy -> -yx
        ...
        self.set_moveToXZ(xpos , ypos[0], t = 0.05)
        #print(ypos[0],ypos[-1])
        ...
        self.set_moveToXZ(xpos , ypos[-1], t = 0) 
#        time.sleep(0.007) 
        time.sleep(0.000)
#        lgwak  = 0
        self.tcspcMeasureFastScanSignal.emit() 
    ...
    def liveview_start(self):
        ...
        #TODO: only at init?
        if self.fast_scan:
            self.tcspcPrepareScanSignal.emit(self.pxTime * self.NofPixels) #TODO check if prepared already
        else:
            self.tcspcPrepareHistoScanSignal.emit(self.pxTime * self.NofPixels) 
...
#        print(lut)
        ...
        for pixelnr in range(self.NofPixels):
            ...
            if pxcount > 0:
                ...
                lifetimes.append(ss.expon.fit(reltimes)[1])
                countprevious = countprevious + pxcount
            else:
                lifetimes.append(0)
            
#            print(lifetimes)
            color = lut.map(lifetimes[-1]- 2.5 / self.maxlifetime) # todochange!!! lut is not [2,2,2]
            color = color.astype(np.float64)/255.
#            print(pxcount)
#            print(maxcount)
            if maxcount > 0:
    ...
    def update_view(self):
        ...
        if self.scantype == 'xy':
            ...
            if self.linedone and self.numberx < self.NofPixels: 
                ...
                if self.fast_scan:
                    [self.lineData, colordata, absphoton, relphoton] = self.fastline_scan(self.data_t,self.data_x[self.numberx], ylist  )
                ...
                if self.lifetimeflag:
                    ...
                    self.maxlifetime = 3 #TODOmake variable
                    self.lifetimeimage[self.numberx] = np.flip(self.make_RGBA_lifetime())
...
#                self.tcspcCloseConnection.emit() 
            ...
            if self.linedone and self.numberx < self.NofPixels: 
                ...
                if self.fast_scan:
                    #self.lineData = self.fastline_scan_allaxis(self.data_t,self.data_x[self.numberx], zlist, "xz"  )
                     [self.lineData, colordata, absphoton, relphoton] = self.fastline_scan_xz(self.data_t,self.data_x[self.numberx], zlist  )
                     self.absphotons.append(absphoton)
                     self.relphotons.append(relphoton)
        ...
        if self.scantype == 'yz':
            ...
            if self.linedone and self.numbery < self.NofPixels: 
                ...
                #print("ypos ", self.data_y[self.numberx])
                if self.fast_scan:
                    [self.lineData, colordata, self.absphotons[self.numbery], self.relphotons[self.numbery] ] = self.fastline_scan_allaxis(self.data_t,self.data_y[self.numbery], zlist, "yz"  )
...
#            
#            print(datetime.now(), '[scan] Shutter not installed...')
#            
#        if val is False:
#            
#            self.shutter_state = False
...
# =============================================================================
        ...
        self.PDNofPixel = 12 # per line
        self.PDmeasurementtime =  4  # ms # check conversions!
        self.PDscanrange = 150 * 10**-3 # um, 100nm # times 2.....
...
#        xs = np.arange(0,length,0.1)
#        ys = np.arange(0,length,0.1)
#        (x,y) = np.meshgrid(xs,ys)
#
#        
...
##        plt.pcolormesh(x,y,Z.reshape((length2,length2)))
##        plt.colorbar()
##        plt.show()
#        
